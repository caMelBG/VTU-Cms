        	...
        	<?php
				...
				foreach($posts as $p) {?>
                    <li>
                        ...
                        <?php if ($instance['display_featured_image'] && has_post_thumbnail($p->ID) ) { ?><?php echo get_the_post_thumbnail($p->ID, array(300,225), array()); ?> <?php } ?>
						
						<?php if ( ($instance['display_date']) ) { ?>
							<div class='post-date'><?php echo get_the_date( get_option( 'date_format' ), $p->ID ); ?></div>
						<?php } ?>
